...
public class Ball : MonoBehaviour //This monobehaviour is unity custom script which allows it to be attached to like the scenes and things?? i think
{

    Vector3 _initialPosition;
    private bool _birdWasLaunched = false;
    private float _timeSittingAround;
    public static int numberOfFailures = -100;

    [SerializeField] private float _launchPower = 20; //We can eactually make this a vector2 form and change the speed on x and y axis separately
    // SerializeField allows us to change this value in te unity gui!

    private void Awake()
    { //Awake is always called when the application starts up
    ...
    private void Update()
    {
        ...
        if (transform.position.y > 25 || transform.position.y < -25 || transform.position.x > 25 || transform.position.x < -25 || _timeSittingAround > 2)
        {
            numberOfFailures++;
